...
const EditProviderById = () => {
  ...
  const fetchProviderData = async () => {
    try {
      ...
      if (response.data.message === 'Providers fetched successfully') {
        const foundProvider = response.data.data.find(provider => provider._id === providerId);
        if (foundProvider) {
  ...
  const handleSaveEdit = async (values, { setSubmitting, setFieldError }) => {
    try {
      ...
      const response = await axios.put(`http://localhost:8000/api/provider/${providerId}`, updateData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      if (error.response?.status === 401) {
        ...
        const errorMessage = error.response?.data?.message || 'Failed to update provider data';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6">
            <h3 className="text-lg leading-6 font-medium text-gray-900">Edit Provider Information</h3>
            <p className="mt-1 max-w-2xl text-sm text-gray-500">Update provider details below</p>
          </div>
          <Formik
                            ...
                            <div className="flex items-center space-x-2">
                              <Field
                                ...
                                className={`flex-1 px-4 py-2 rounded-lg border-2 ${
                                  errors[key] && touched[key] ? 'border-red-500' : 'border-gray-200'
