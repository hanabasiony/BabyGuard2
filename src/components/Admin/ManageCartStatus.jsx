    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.patch(
                `http://localhost:8000/api/carts/status/admin/${cartId}`,
                { status },
                {
                    headers: {
            ...
            toast.error(error.response?.data?.message || 'Failed to update cart status');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl mx-auto">
                <div className="bg-white rounded-lg shadow-md p-8">
                    <div className="mb-8">
                        <h2 className="text-2xl font-bold text-gray-900">Manage Cart Status</h2>
                        <p className="mt-2 text-sm text-gray-600">
                            Update the status of a cart by entering its ID and selecting the new status.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        {/* Cart ID */}
                        <div>
                            <label htmlFor="cartId" className="block text-sm font-medium text-gray-700 mb-1">
                                Cart ID
                            </label>
                            <input
                                type="text"
