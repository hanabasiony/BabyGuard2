...
import { User, Package, MapPin, CreditCard, Calendar, ArrowLeft, Search, Filter, ChevronDown } from 'lucide-react';
...
const CartDetails = () => {
    ...
    const statusOptions = [
        ...
        { value: 'Waiting for cash payment', label: 'Waiting for Cash', color: 'orange' },
    ...
    const fetchUserData = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:8000/api/carts/admin', {
                headers: {
    ...
    const handleStatusChange = async (cartId, newStatus) => {
        try {
            ...
            const token = localStorage.getItem('token');
            await axios.patch(`http://localhost:8000/api/carts/status/admin/${cartId}`, 
                { status: newStatus },
                {
                    headers: {
                        Authorization: `Bearer ${token}`
            ...
            toast.success(`Order #${cartId.slice(-6)} status updated to ${newStatus}`);
        } catch (err) {
                        ...
                        <div className="flex justify-between items-start mb-4">
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="font-medium">Order #{cart._id.slice(-6)}</span>
                                </div>
                                    ...
                                    {updatingStatuses[cart._id] ? (
                                        ...
                                        <>
                                            <select
                                                ...
                                                className={`appearance-none px-3 py-1 rounded-full text-sm pr-8 cursor-pointer focus:outline-none focus:ring-2 focus:ring-pink-500 ${
                        ...
                        <div className="mt-4 border-t pt-4">
                            ...
                            <div className="space-y-3">
                                {cart.products.map((product) => (
                                    <div key={product._id} className="flex items-center space-x-4 bg-gray-50 p-3 rounded">
                                        <img 
                                            src={product.image} 
                                            alt={product.name}
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                Age: {product.requiredAge} • Quantity: {product.quantity} • Price: ${product.price}
                                            </p>
                                        </div>
                                    </div>
                                ))}
